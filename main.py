...
class SubscriptionAggregatorPlugin(Star):
    def __init__(self, context: Context, config: AstrBotConfig):
        ...
        self._http_watch_task: asyncio.Task | None = None
    ...
    async def on_astrbot_loaded(self):
        ...
        self._http_watch_task = asyncio.create_task(self._http_watch_loop())
    ...
    async def terminate(self):
        ...
        if self._http_watch_task:
            self._http_watch_task.cancel()
            await asyncio.gather(self._http_watch_task, return_exceptions=True)
    ...
    async def list_cmd(self, event: AstrMessageEvent):
        ...
        for index, source in enumerate(sources, start=1):
            ...
            detail = "详细变化开" if source.get("notify_changes", True) else "仅摘要"
            lines.append(f"{index}. {source.get('name', '未命名')} | {kind} | {state} | {detail}")
        yield event.plain_result("\n".join(lines))
    ...
    async def _ensure_http_server(self) -> None:
        if not self.config.get("http_enable", True) or self._runner_active():
            return
        try:
            await self.http_server.start()
            self._http_start_error = ""
            self._append_log("warning", "http server recovered", host=self.http_server.host, port=self.http_server.port)
        except Exception as exc:
            self._http_start_error = f"{type(exc).__name__}: {exc}"
            self._append_log("error", "http server recovery failed", error=type(exc).__name__)

    async def _http_watch_loop(self):
    ...
    def _format_refresh_report(self, reason: str, report: RefreshReport) -> str:
        ...
        change_message = format_change_message(
            ...
            report.updated,
            report.removed,
            {source.get("name", "") for source in self._sources() if source.get("notify_changes", True)},
        )
        if change_message:
