...
The provider files are MRS and are refreshed every 86400 seconds. The profile includes only ads and mainland China categories. All traffic uses the single manually controlled `PROXY` select group; it does not create URL-test, region, or application groups.
...
Provider URLs are public and were checked with HTTP HEAD during development. The profile was loaded by the local Mihomo Alpha binary, and both MRS provider files were downloaded as non-empty files in an isolated temporary runtime directory.
