...
def test_vmess_tcp_transport_and_ps_name():
    payload = {"v": "2", "ps": "🇰🇷 Korea AWS｜薛定谔的三网", "add": "example.com", "port": "443", "id": "uuid", "aid": "0", "scy": "auto", "tls": "tls", "net": "tcp"}
    raw = "vmess://" + base64.urlsafe_b64encode(json.dumps(payload, ensure_ascii=False).encode()).decode().rstrip("=")
    proxy = convert_share_link(raw, "vmess-tcp")
    assert proxy["type"] == "vmess"
    assert proxy["name"] == "🇰🇷 Korea AWS｜薛定谔的三网"
    assert "network" not in proxy


def test_vmess_grpc_transport():
